...
public class CreditCardNumberGenerator {
     ...
     * Ref: http://codytaylor.org/2009/11/this-is-how-credit-card-numbers-are-generated.html
     *
     * @param bin
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < 1; i ++) {
            cardList.add(g.generate("5254", 16));
        }

        System.out.println("Total Cards - " + cardList.size());

        CreditCardNumberValidator v = new CreditCardNumberValidator();
        cardList.stream().forEach( s -> System.out.println(s + " - " + (v.check(Long.parseLong(s)) ? "Valid" : "Invalid")));


        //cardList.stream().filter(s -> v.validitychk(Long.parseLong(s))).collect(Collectors.toList()).stream().forEach( a -> System.out.println(a));
    }
